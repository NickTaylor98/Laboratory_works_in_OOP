...
Student::Student(string surname, string name, string second_name, Data birthday, Adress adress, string tel, string fac, us group, us course, bool sex)
{
	...
	Telephone = tel;
	Faculty = (fac.size() < MAX_SIZE_OF_FACULTY? fac:  throw new Exeption_Faculty("Длинное название факультета"));
...
void Student::setFaculty(string faculty) { Faculty = (faculty.size() < MAX_SIZE_OF_FACULTY ? faculty : throw new Exeption_Faculty("Длинное название факультета")); }
void Student::setCourse(us course) { Course = course > MaxCourse ? throw new Exeption_Course("Ошибка с номером курса"): course; }
void Student::setGroup(us group) { assert(group<MaxGroup); Group = group; }
...
{
	if (!(*fin).is_open()) throw new Exeption_Fstream("Ошибка в файловом потоке");
	else
